# Ieum Mobility Support — 지하철 접근성·혼잡도 예보

이동약자(휠체어 이용자)를 위한 서울 지하철 **혼잡도 예보 및 접근성 기반 이동 의사결정 지원** 프로젝트(Apex).
공개 API 수집, 혼잡도 예측, 역사 시설·동선 데이터를 연결해 사용자가 “언제, 어떤 경로로 이동할지” 판단할 수 있게 돕는 웹 프로토타입이다.
